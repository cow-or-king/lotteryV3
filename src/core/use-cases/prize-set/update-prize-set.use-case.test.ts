...
import { UpdatePrizeSetUseCase } from './update-prize-set.use-case';
import type { PrizeSetRepository } from '@/core/ports/prize-set.repository';
import type { BrandRepository } from '@/core/ports/brand.repository';

describe('UpdatePrizeSetUseCase', () => {
  let useCase: UpdatePrizeSetUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new UpdatePrizeSetUseCase(mockPrizeSetRepository, mockBrandRepository);
  });

  it('should update a prize set', async () => {
    const userId = 'user-123';
    const input = { id: 'prize-set-123', name: 'Nouveau nom' };
    ...
    vi.mocked(mockPrizeSetRepository.update).mockResolvedValue({
      ...mockPrizeSet,
      name: input.name,
    });
